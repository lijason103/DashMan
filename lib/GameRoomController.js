...
class GameRoomController {
    ...
    handle_join_room() {
        this.socket.on('JOIN_ROOM', room_id => {
            if (room_id && room_id.startsWith(GAME_ROOM) && this.io.sockets.adapter.rooms.hasOwnProperty(room_id)) {
                if (GameRoomController.game_rooms[room_id] && GameRoomController.game_rooms[room_id].status == 'IN-GAME') return
                ...
                this.io.in(room_id).emit('GAME_ROOM', {
                    id: room.id,
                    status: room.status,
    ...
    handle_leave_room() {
        this.socket.on('LEAVE_ROOM', () => {
            ...
            if (room) {
                this.io.in(this.current_room_id).emit('GAME_ROOM', {
                    id: room.id,
                    status: room.status,
                    clients: room.clients,
                    host: room.clients[0]
